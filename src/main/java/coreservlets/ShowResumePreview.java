...
public class ShowResumePreview
  extends HttpServlet {

  private static final long serialVersionUID = -616594945577369689L;
  ...
  /**
   * Shows a preview of the submitted resume. Takes the font information and builds an
   * HTML style sheet out of it, then takes the real resume information and presents it
   * formatted with that style sheet.
   */
  private void showPreview(HttpServletRequest request, PrintWriter out) {
    ...
    out.println(docType + "<HTML><HEAD><TITLE>Resume for "
                + name
                + "</TITLE>\n"
                + makeStyleSheet(headingFont,
                                 headingSize,
                                 bodyFont,
                                 bodySize,
                                 fgColor,
  ...
  /**
   * Builds a cascading style sheet with information on three levels of headings and
   * overall foreground and background cover. Also tells Internet Explorer to change color
   * of mailto link when mouse moves over it.
   */
  ...
  /**
   * Replaces null strings (no such parameter name) or empty strings (e.g., if textfield
   * was blank) with the replacement. Returns the original string otherwise.
   */
  ...
  /**
   * Replaces null strings, empty strings, or the string "default" with the
   * replacement.Returns the original string otherwise.
   */
  ...
  /**
   * Takes a string representing an integer and returns it as an int. Returns a default if
   * the string is null or in an illegal format.
   */
  ...
  /**
   * Given "Java,C++,Lisp", "Java C++ Lisp" or "Java, C++, Lisp", returns
   * <UL>
   * <LI>Java
   * <LI>C++
   * <LI>Lisp
